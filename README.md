...
**Runtime input validation** — service names in API requests are used as map keys, never interpolated into shell commands. Port numbers are validated by the OS `net.Listen` call. Spec fields are parsed by Go's YAML decoder with no custom deserialization.
